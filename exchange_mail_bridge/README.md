...
## Second mailbox: Aufschaltung AutoReply

The Aufschaltung auto-reply uses the same bridge logic as the Ausschreibung setup, but it must stay separate:

```text
Aufschaltung Exchange mailbox -> exchange-mail-bridge-aufschaltung.yml -> Apps Script Web App -> Aufschaltung Sheet -> autoReplyAufschaltung()
```
...
```text
.github/workflows/exchange-mail-bridge-aufschaltung.yml
```

Use separate GitHub repository secrets so the Ausschreibung mailbox is not overwritten:

```text
AUFSCHALTUNG_EWS_URL=https://mail.wackerhagengruppe.de/EWS/Exchange.asmx
AUFSCHALTUNG_EWS_EMAIL=<aufschaltung mailbox address>
AUFSCHALTUNG_EWS_USERNAME=<aufschaltung mailbox username>
AUFSCHALTUNG_EWS_PASSWORD=<set only in GitHub Secrets>
AUFSCHALTUNG_EWS_AUTH_TYPE=NTLM
AUFSCHALTUNG_EWS_VERIFY_TLS=true
AUFSCHALTUNG_APPS_SCRIPT_WEBAPP_URL=<Aufschaltung Apps Script /exec URL>
AUFSCHALTUNG_BRIDGE_TOKEN=<same token as Apps Script property AUFSCHALTUNG_BRIDGE_TOKEN>
```

The Aufschaltung workflow has a config guard. If any of these secrets are missing, the scheduled run skips cleanly without touching mail or Sheets.

Before enabling real sends:

1. In Apps Script, run `createAufschaltungSheetAndSetPropertyNow()` once.
2. Set script property `AUFSCHALTUNG_BRIDGE_TOKEN` to the same value as the GitHub secret.
3. Deploy the Apps Script as a Web App and copy the `/exec` URL into `AUFSCHALTUNG_APPS_SCRIPT_WEBAPP_URL`.
4. Run the Aufschaltung workflow manually with `dry_run_summary=true`.
5. Run a normal import and check the Google Sheet rows/status columns.
6. Run `checkAutoReplyAufschaltungSetup()`.
7. Only after the test is clean, run `setupAutoReplyAufschaltungTrigger()`.
